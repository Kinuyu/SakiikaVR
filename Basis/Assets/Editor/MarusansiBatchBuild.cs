...
///             -executeMethod MarusansiBatchBuild.BuildWindows
/// </summary>
public static class MarusansiBatchBuild
{
    public static void BuildWindows()
    {
        try
        {
            ...
            Debug.Log("[MarusansiBatchBuild] Building Addressables...");
            ...
            {
                scenes = scenes,
                locationPathName = "../Builds/Windows/MarusansiVR.exe",
                target = BuildTarget.StandaloneWindows64,
                subtarget = (int)StandaloneBuildSubtarget.Player,
                options = BuildOptions.None,
            };

            Debug.Log($"[MarusansiBatchBuild] Building player with {scenes.Length} scene(s)...");
            BuildReport report = BuildPipeline.BuildPlayer(options);
            BuildSummary summary = report.summary;
            Debug.Log($"[MarusansiBatchBuild] Result={summary.result} Errors={summary.totalErrors} Warnings={summary.totalWarnings} Size={summary.totalSize} Output={summary.outputPath}");

            EditorApplication.Exit(summary.result == BuildResult.Succeeded ? 0 : 1);
        }
        catch (Exception ex)
        {
            Debug.LogError($"[MarusansiBatchBuild] FAILED: {ex}");
