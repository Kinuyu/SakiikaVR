...
namespace Basis.Scripts.Networking.Marusansi
{
    ...
    /// <summary>
    /// P2P (player-hosted) world sessions for MarusansiVR. Selecting a world in the
    ...
    /// </summary>
    public static class MarusansiWorldSession
    {
        ...
        public const string HomeWorldUrlFile = "MarusansiHomeWorld.BAS";
        ...
        private static MarusansiTurnClient _activeTurn;
        ...
        public static async Task StartWorldSessionAsync(BasisDataStoreItemKeys.ItemKey item, string worldDisplayName, WorldSessionVisibility visibility, bool showResultDialog = true)
        {
            ...
            {
                ShowDialog(BasisLocalization.Get("marusansi.session.misskeyRequired.title"),
                    BasisLocalization.Get("marusansi.session.misskeyRequired.body"));
                return;
            }
            ...
            try
            {
                ...
                if (!hostConnected)
                {
                    BasisConnectionService.ReportConnectionError(BasisLocalization.Get("marusansi.session.hostFailed"));
                    return;
                }
                ...
                // in-process server.
                (bool symmetric, IPEndPoint srflx) = await MarusansiStun.DetectNatAsync(NetworkServer.Server);
                ...
                if (symmetric)
                {
                    _activeTurn = new MarusansiTurnClient();
                ...
                if (visibility == WorldSessionVisibility.Public)
                {
                    ...
                    {
                        noteFailureLine = "\n\n" + BasisLocalization.Get("marusansi.session.noPublicIp");
                    }
                    else
                    {
                        ...
                        {
                            noteFailureLine = "\n\n" + BasisLocalization.Get("marusansi.session.announceFailed");
                        }
                        else
                        {
                            // Poll the note's replies for guests. In relay mode we permit
                            // each guest on the TURN allocation; otherwise we punch its NAT.
                            MarusansiHolePunch.StartHost(ActiveNoteId, usingRelay ? _activeTurn : null);
                        }
                        InstallQuitHook();
                    }
                }
                ...
                if (showResultDialog)
                {
                    ...
                    string title = visibility == WorldSessionVisibility.Public
                        ? BasisLocalization.Get("marusansi.session.startedPublic.title")
                        : BasisLocalization.Get("marusansi.session.startedInvite.title");
                    string body = string.Format(BasisLocalization.Get("marusansi.session.started.body"),
        ...
        private static async Task ReuseSessionSwapWorldAsync(BasisDataStoreItemKeys.ItemKey item, string worldDisplayName, WorldSessionVisibility visibility, bool showResultDialog)
        {
            ...
            if (visibility == WorldSessionVisibility.Public)
            {
                ...
                if (string.IsNullOrEmpty(ActiveNoteId) && !string.IsNullOrEmpty(conn))
                {
                    ...
                    if (!string.IsNullOrEmpty(ActiveNoteId))
                    {
                        MarusansiHolePunch.StartHost(ActiveNoteId, _activeTurn);
                        InstallQuitHook();
                    }
                }
            }
        ...
        public static async Task JoinFromConnectionStringAsync(string connectionString)
        {
            ...
            {
                ShowDialog(BasisLocalization.Get("marusansi.session.joinFailed.title"),
                    string.Format(BasisLocalization.Get("marusansi.session.badConnection.body"), connectionString ?? string.Empty));
                return;
            }
        ...
        public static async Task JoinPublicWorldAsync(WorldAnnouncePayload payload, string noteId)
        {
            ...
            {
                ShowDialog(BasisLocalization.Get("marusansi.session.joinFailed.title"),
                    string.Format(BasisLocalization.Get("marusansi.session.badConnection.body"), payload.conn ?? string.Empty));
                return;
            }
        ...
        private static async Task RunGuestHolePunchAsync(string noteId, IPEndPoint hostEndpoint)
        {
            try
            {
                ...
                IPEndPoint guestPublic = await MarusansiStun.DiscoverAsync(client);
                await MarusansiHolePunch.GuestSignalAndPunchAsync(noteId, hostEndpoint, guestPublic);
            }
            catch (Exception ex)
            {
