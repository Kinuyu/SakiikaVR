...
namespace Basis.BasisUI
{
    public partial class LibraryProvider : BasisMenuActionProvider<BasisMainMenu>
    {
        ...
        public static async Task RefreshTabAsync(Page page, bool clearSearch = false)
        {
            ...
            if (TryConvert(page, out BundledContentHolder.Mode mode))
            {
                try
                {
                    ...
                    // MarusansiVR: pinned embedded items (Personal Mirror, Photo
        ...
        public static void ShowItemOverlay(BasisDataStoreItemKeys.ItemKey item)
        {
            ...
            // MarusansiVR: worlds are opened as hosted sessions (Invite/Public), so
            ...
            // MarusansiVR: worlds are opened via the Invite/Public session buttons
            ...
            // ── MarusansiVR: player-hosted P2P sessions ──────────────────────
            ...
            if (p2pHostable)
            {
                ...
                void UpdateHomeButtonTitle() => homeButton.Descriptor.SetTitle(BasisLocalization.Get(
                    MarusansiWorldSession.IsHomeWorld(item) ? "marusansi.home.unset" : "marusansi.home.set"));
                UpdateHomeButtonTitle();
                homeButton.OnClicked += () =>
                {
                    MarusansiWorldSession.ToggleHomeWorld(item);
                    UpdateHomeButtonTitle();
                };
                ...
                inviteButton.OnClicked += async () =>
                {
                    ...
                    existingItemDialog.IsBusy = true;
                    existingItemDialog.CloseWithResult(null);
                    await MarusansiWorldSession.StartWorldSessionAsync(item, worldDisplayName, WorldSessionVisibility.Invite);
                };
                ...
                publicButton.OnClicked += async () =>
                {
                    ...
                    existingItemDialog.IsBusy = true;
                    existingItemDialog.CloseWithResult(null);
                    await MarusansiWorldSession.StartWorldSessionAsync(item, worldDisplayName, WorldSessionVisibility.Public);
                };
            }
        }
