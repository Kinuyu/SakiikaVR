...
using Basis.Scripts.Networking.Marusansi;
using System;
...
{
    /// <summary>
    /// Main-menu Worlds panel for MarusansiVR. Replaces the retired Servers panel:
    ...
    /// </summary>
    public class MarusansiWorldsProvider : BasisMenuActionProvider<BasisMainMenu>
    {
        [RuntimeInitializeOnLoadMethod]
        public static void AddToMenu()
        {
            BasisMenuBase<BasisMainMenu>.AddProvider(new MarusansiWorldsProvider());
        }
        ...
        public override void RunAction()
        {
            ...
            _emptyState.SetTitle(BasisLocalization.Get("marusansi.worlds.empty"));
        ...
        private void BuildHeader(RectTransform container)
        {
            ...
            _refreshButton.Descriptor.SetTitle(BasisLocalization.Get("marusansi.worlds.refresh"));
        ...
        private void UpdateLoginHeaderButton()
        {
            ...
            _loginHeaderButton.Descriptor.SetTitle(MisskeyService.IsLoggedIn
                ? string.Format(BasisLocalization.Get("marusansi.worlds.loggedInAs"), MisskeyService.Username, MisskeyService.InstanceUrl)
                : BasisLocalization.Get("marusansi.worlds.login"));
        }
        ...
        private async Task RefreshWorldsAsync()
        {
            ...
            {
                ClearRows();
                _emptyState.SetTitle(BasisLocalization.Get("marusansi.worlds.querying"));
        ...
        private void AddWorldRow(WorldAnnouncePayload payload, MisskeyNote note, DateTime createdUtc)
        {
            ...
            group.SetTitle(string.IsNullOrEmpty(payload.name)
                ? BasisLocalization.Get("marusansi.worlds.unnamed")
                : payload.name);
            ...
            group.SetDescription(string.Format(BasisLocalization.Get("marusansi.worlds.row.description"),
            ...
            joinButton.Descriptor.SetTitle(BasisLocalization.Get("marusansi.worlds.join"));
            string joinNoteId = note.id;
            joinButton.OnClicked += () => _ = MarusansiWorldSession.JoinPublicWorldAsync(payload, joinNoteId);
        ...
        private static string FormatTimeAgo(DateTime createdUtc)
        {
            ...
            {
                return string.Format(BasisLocalization.Get("marusansi.worlds.minutesAgo"), (int)age.TotalMinutes);
            }
            return string.Format(BasisLocalization.Get("marusansi.worlds.hoursAgo"), (int)age.TotalHours);
        }
        ...
        private void BuildMisskeySection(RectTransform container)
        {
            ...
            _misskeyToggle.SetTitle(BasisLocalization.Get("marusansi.worlds.misskey"));
            ...
            _accountStatus.SetDescription(BasisLocalization.Get("marusansi.worlds.misskey.description"));
            UpdateAccountStatus();

            _instanceField = PanelTextField.CreateNewEntry(container);
            _instanceField.Descriptor.SetTitle(BasisLocalization.Get("marusansi.worlds.instance"));
            ...
            _loginButton.Descriptor.SetTitle(BasisLocalization.Get("marusansi.worlds.login"));
            _loginButton.OnClicked += () => _ = LoginAsync();

            _logoutButton = PanelButton.CreateNew(accountActions);
            _logoutButton.Descriptor.SetTitle(BasisLocalization.Get("marusansi.worlds.logout"));
        ...
        private async Task LoginAsync()
        {
            ...
            try
            {
                ...
                SetAccountStatus(BasisLocalization.Get("marusansi.worlds.loginWaiting"));
                ...
                {
                    SetAccountStatus(BasisLocalization.Get("marusansi.worlds.loginFailed"));
                }
            }
            catch (Exception ex)
            {
        ...
        private void UpdateAccountStatus()
        {
            SetAccountStatus(MisskeyService.IsLoggedIn
                ? string.Format(BasisLocalization.Get("marusansi.worlds.loggedInAs"), MisskeyService.Username, MisskeyService.InstanceUrl)
                : BasisLocalization.Get("marusansi.worlds.notLoggedIn"));
        }
